...
extension ToDoItem
{
    public class func getMockData() -> [ToDoItem]
    {
        return [
            ...
            ToDoItem(title: "Paint the deck"),
            ToDoItem(title: "Pay bills"),
            ToDoItem(title: "Feed the dog")
        ]
    }
}
...
extension Collection where Iterator.Element == ToDoItem
{
    ...
    static func readFromPersistence() throws -> [ToDoItem]
    {
        if let url = persistencePath(), let data = (try Data(contentsOf: url) as Data?)
        {
            ...
            {
                throw NSError(domain: "com.kuharich.ToDoTaskList", code: 11, userInfo: nil)
            }
        }
        else
        {
            throw NSError(domain: "com.kuharich.ToDoTaskList", code: 12, userInfo: nil)
        }
    }
}
